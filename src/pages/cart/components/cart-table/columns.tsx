...
export const columns: ColumnDef<Cart>[] = [
        ...
        cell: ({ row }) => (
            <HoverCard>
                <HoverCardTrigger asChild>
                    <div className='pl-2.5'>
                        <div className='h-[32px] w-[46px]'>
                            <ImageWithSkeleton
                                src={row.original?.stock_product.shop_product.image}
                                alt={
                                    row.original?.stock_product.shop_product.product
                                        .ukr_name
                                }
                ...
                >
                    <ImageWithSkeleton
                        src={row.original?.stock_product.shop_product.image}
                        alt={row.original?.stock_product.shop_product.product.ukr_name}
    ...
    {
        accessorFn: (row) => row.stock_product.shop_product.product?.ukr_name,
        header: 'Назва',
        cell: ({ row }) => (
            <div className='flex flex-col'>
                <span className='truncate text-foreground'>
                    {row.original?.stock_product.shop_product.product?.ukr_name}
                </span>
                <div className='flex items-center gap-x-1'>
                    <img
                        src={
                            row.original?.stock_product.shop_product.producer?.country
                                ?.flag
                        }
                        alt={row.original?.stock_product.shop_product.producer?.name}
                        className='size-4'
                    />
                    <span className='truncate text-xs'>
                        {row.original?.stock_product.shop_product.producer?.name}
                    </span>
                </div>
            </div>
        ),
    ...
    },
    {
        ...
        header: 'Висота',
        cell: ({ row }) => (
            <HeightInfo height={row.original?.stock_product?.shop_product?.height} />
        ),
        size: 60
    },
    {
        ...
        cell: ({ row }) => (
            <WeighDiameterInfo
                weight={row.original?.stock_product?.shop_product?.weight_size}
                diameter={row.original?.stock_product?.shop_product?.diameter}
            />
        ),
        size: 100
    },
    ...
    },
    {
        ...
        size: 20,
        cell: ({ row }) => {
            return row.original?.stock_product?.promotion ? (
                <svg
                    width='20'
                    height='20'
                    viewBox='0 0 20 20'
    ...
    },
    {
        ...
        header: '',
        size: 60,
        cell: ({ row }) => <DiscountCell cart={row.original} />
    },
    {
        header: 'Кількість',
        accessorKey: 'quantity',
        cell: ({ row }) => <QuantityCell cart={row.original} />,
        size: 165
    },
    {
        header: 'Замовлення',
        accessorKey: 'order',
        cell: ({ row }) => <OrderCell cart={row.original} />,
        size: 90
    },
    {
        accessorKey: 'actions',
        header: '',
        cell: ({ row }) => <CartActionsCell cart={row.original} />,
        size: 95
    }
]
